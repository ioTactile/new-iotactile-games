...
describe("LockDiceUsecase", () => {
	...
	it("retourne INVALID_DICE_INDEX si diceIndex < 0 ou > 4", async () => {
		const usecase = new LockDiceUsecase(
			sessionRepo,
			playerRepo,
			stateRepo,
		...
		if (!r1.ok) expect(r1.error.message).toBe("INVALID_DICE_INDEX");
		...
		if (!r2.ok) expect(r2.error.message).toBe("INVALID_DICE_INDEX");
		expect(sessionRepo.findById).not.toHaveBeenCalled();
	});
	...
	it("bascule locked sur le dé à diceIndex et broadcast", async () => {
